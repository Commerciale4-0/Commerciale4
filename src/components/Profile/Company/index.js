...
export default class ProfileCompany extends Component {
    constructor(props) {
        ...
        let selectedType = COMPANY_TYPES.filter(
            type => type.label === props.profile.typeOfCompany
        );

        let productPhotos = [];
        props.profile.productPhotos &&
            props.profile.productPhotos.map(photo => {
                productPhotos.push(process.env.REACT_APP_AWS_PREFIX + photo);
            });

        let originPhotos = {
            background: props.profile.background
                ? process.env.REACT_APP_AWS_PREFIX + props.profile.background
                : null,
            logo: props.profile.logo
                ? process.env.REACT_APP_AWS_PREFIX + props.profile.logo
                : null,
    ...
    handleClickSave = async e => {
        ...
        let dataToSave = {
            ...
            stringData: {
                ...
                employees: this.refEmployee.current.value,
                revenues: this.refRevenue.current.value,
            ...
            imageData: {
                background:
                    coverImage !== originPhotos.background ? coverImage : null,
        ...
        if (
            originPhotos.background &&
            coverImage &&
            coverImage.search(prefix) === -1
        ) {
            let path = originPhotos.background.substr(
                originPhotos.background.search(prefix) + prefix.length,
                originPhotos.background.length
            );
            removedPhotos.push(path);
        }

        if (originPhotos.logo && logoImage && logoImage.search(prefix) === -1) {
            let path = originPhotos.logo.substr(
                originPhotos.logo.search(prefix) + prefix.length,
                originPhotos.logo.length
            );
            removedPhotos.push(path);
        }
        ...
        for (let i in originPhotos.productPhotos) {
            ...
            if (removed) {
                let path = originPhoto.substr(
                    originPhoto.search(prefix) + prefix.length,
                    originPhoto.length
                );
                removedPhotos.push(path);
            }
        ...
        for (let i in productImages) {
            ...
            if (index === -1) {
                ...
                newImages.push(
                    photo.substr(index + prefix.length, photo.length)
                );
            }
        }
        ...
        await requestAPI("/user/profile/edit", "POST", dataToSave).then(res => {
            ...
            if (res.status === 1) {
                if (res.data) {
                    ...
                    if (res.data.productPhotos) {
                        let productPhotos = [];
                        res.data.productPhotos.map(photo => {
            ...
            <div className={selectedTab === 0 ? "d-block" : "d-none"}>
                ...
                <div>
                    <textarea
                        ...
                        className={
                            selectedIntroLang === 2 ? "d-block" : "d-none"
                        }
                    ></textarea>
                    <textarea
                        ref={this.refIntroIt}
                        ...
                        className={
                            selectedIntroLang === 1 ? "d-block" : "d-none"
                        }
                    />
                </div>
                ...
                <div>
                    <textarea
                        ...
                        className={
                            selectedWhatDoLang === 2 ? "d-block" : "d-none"
                        }
                    />
                    <textarea
                        ...
                        className={
                            selectedWhatDoLang === 1 ? "d-block" : "d-none"
                        }
                    />
                </div>
                <div className="char-limit">Max 500 characters</div>
                ...
                <div className="mt-3">
                    <div className="info-row">
                        ...
                        <input
                            ref={this.refEmployee}
                            defaultValue={profile && profile.employees}
                        />
                    </div>
                    <div className="info-row">
                        <span>Revenues:</span>
                        <input
                            ref={this.refRevenue}
                            defaultValue={profile && profile.revenues}
                        />
                    </div>
                    <div className="info-row">
                        <span>ISO:</span>
            ...
            <div className={`mt-4 ${selectedTab === 1 ? "d-block" : "d-none"}`}>
                    ...
                    <div className="photos-panel">
                        {productImages.map((image, index) => (
                            <div key={index} className="photo">
                                ...
                                <button
                                    onClick={() =>
                                        this.handleRemoveProductImage(index)
                                    }
                                >
                ...
                <div>
                    <textarea
                        ...
                        defaultValue={profile.productDetail}
                    />
                </div>
            </div>
        );
        ...
        const contactsPanel = (
            <div className={`mt-4 ${selectedTab === 2 ? "d-block" : "d-none"}`}>
                ...
                <div className="info-row">
                    <span>2nd email:</span>
                    <input
                        ref={this.ref2ndEmail}
                        defaultValue={profile.company2ndEmail}
                    />
                </div>
            </div>
        );
        ...
        return (
            <div className="company-view">
                ...
                <button
                    ...
                    onClick={() => this.handleBrowseFile(1, IMAGE_LOGO)}
                >
                    <i className="fa fa-upload" />
                </button>
                <button
                    className="secondary btn-change-cover"
                    onClick={() => this.handleBrowseFile(0, IMAGE_COVER)}
                >
                    <i className="fa fa-upload pr-2" />
                    Choose system photo
                </button>
                ...
                {targetToCrop &&
                    targetToCrop.info &&
                    targetToCrop.info.image && (
                        <ImageCropper
                            options={targetToCrop.info}
