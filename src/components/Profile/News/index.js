...
export default class ProfileNews extends Component {
	...
	render() {
		const { photoFileName, posts, isProcessing, alertData } = this.state;
		return (
			<div className="news-view">
				...
				<div>
					<input className="w-100" ref={this.refTitle} />
				</div>
				<div className="mt-3 mb-1">What is new</div>
				<div>
					<textarea ref={this.refDescription} />
				</div>
				<div>
					<button
				...
				<div>
					{posts.map((post, index) => (
						<PostItem
							key={index}
							data={post}
							handleDelete={() => this.handleDeleteNews(post)}
						/>
					))}
				</div>
