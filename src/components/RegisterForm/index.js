...
export default class RegisterForm extends Component {
	...
	handleClickDone = async (e) => {
		...
		try {
			...
			/* it's for just test */
			this.setState({ step: 3 });
			this.setAlertData(1, `We've sent an email to ${data.pec} to verify your account. Please check your email inbox to coutinue.`);
			/**********************/
	...
	render() {
		const { step, atecoCodes, selectedCity, selectedCode, checkValidCity, checkValidCode, alertData, isProcessing } = this.state;
		...
		const stepOne = (
			...
			>
				...
				<Row className="justify-content-center mb-3">
					<Col md={6}>
						<input type="text" name="officialName" placeholder="Official Name" ref={this.refName} />
					</Col>
				</Row>
				<Row className="justify-content-center mb-3">
					<Col md={6}>
						<MySelect value={selectedCity} onChange={this.handleCityChange} options={CITIES} placeholder="City" checkValid={checkValidCity} />
					</Col>
				</Row>
				<Row className="justify-content-center mb-3">
					<Col md={6}>
						<input type="text" name="vatNumber" placeholder="VAT number" ref={this.refVAT} />
					</Col>
				</Row>
				<Row className="justify-content-center mb-3">
					<Col md={6}>
				...
				<Row className="mb-3">
					<Col md={{ span: 6, offset: 3 }}>
						<input type="text" name="vatNumber" placeholder="PEC" ref={this.refPEC} />
					</Col>
					<Col md={3} className="info-hint">
						<i className="fa fa-info-circle pr-1" />
						This inbox will be use only to certify the attendibility of the user
					</Col>
				</Row>
				<Row className="justify-content-center mb-3">
					<Col sm={3} xs={12}>
		...
		const stepTwo = (
			...
			>
				...
				<Row className="mb-3">
					<Col md={{ span: 6, offset: 3 }}>
						<input type="text" name="email" placeholder="Email address" ref={this.refEmail} />
					</Col>
					<Col md={3} className="info-hint">
						<i className="fa fa-info-circle pr-1" />
						This inbox will be use to receive informations related to the service
					</Col>
				</Row>
				<Row className="justify-content-center mb-3">
					<Col md={6}>
				...
				<Row className="justify-content-center mb-3">
					<Col md={6}>
						...
						<label className="label-checkbox" htmlFor="rememberme">
							In order to continue, confirm to accept the Privacy policy and Terms and conditions
						</label>
					</Col>
				</Row>
				<Row className="justify-content-center mb-3">
				...
				<div className="text-center mb-3">After "Complete registration", you will receive a confirmation message in your PEC inbox</div>
			</div>
		);

		const stepThree = (
			<div className="d-flex justify-content-center">
				<button onClick={() => this.handleClickGoHome()}>Go to Home</button>
			</div>
		);
