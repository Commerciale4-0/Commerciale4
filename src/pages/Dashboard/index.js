...
import {
  ...
  SESSION_LOGGED_COMPANY,
  SESSION_FILTER,
  ORDERS,
  SESSION_LOCAION_ERROR,
} from "../../utils";
...
export default class Dashboard extends Component {
  ...
  getLocationAndPull = () => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        this.positionSuccessHandler,
        this.positionErrorHandler,
  ...
  positionErrorHandler = (err) => {
    if (!sessionStorage.getItem(SESSION_LOCAION_ERROR)) {
      ...
      alert(
        "Please make sure to be enabled location service. This is needed to search companies by location."
      );
    }
    this.pullCompanies(
      this.state.activePage,
  ...
  pullCompanies = async (pageNumber, count, sort, myLocation) => {
    ...
    if (filter) {
      if (filter.region && filter.region.value) {
        ...
        if (filter.city && filter.city.value) {
          ...
          if (filter.radius) {
            ...
            let geocode = await geocodeByAddress(
              `${filter.city.label}, ${filter.region.label}`
            );
            if (geocode) {
      ...
      if (filter.type && filter.type.value)
        searchFilter.type = filter.type.value;
      if (filter.employeeMin && filter.employeeMin.value)
        searchFilter.employeeMin = numberFromStringWithUnit(
          filter.employeeMin.label
        );
      if (filter.employeeMax && filter.employeeMax.value)
        searchFilter.employeeMax = numberFromStringWithUnit(
          filter.employeeMax.label
        );
      if (filter.revenueMin && filter.revenueMin.value)
        searchFilter.revenueMin = numberFromStringWithUnit(
          filter.revenueMin.label
        );
      if (filter.revenueMax && filter.revenueMax.value)
        searchFilter.revenueMax = numberFromStringWithUnit(
          filter.revenueMax.label
        );
      ...
      if (filter.tags && filter.tags.length) {
        ...
        lang === "en"
          ? (searchFilter.tags = { en: tags })
          : (searchFilter.tags = { it: tags });
      }
    }
    if (pageNumber === 1) {
    ...
    let resultOfCompanies = showCompany
      ? result.specifiedCompany
      : result.companies;
    let resultOfRegisteredCompanies = [];
    let resultOfUnRegisteredCompanies = [];
    for (let i in resultOfCompanies) {
      if (resultOfCompanies[i].account.permission === 2) {
        resultOfRegisteredCompanies.push(resultOfCompanies[i]);
      } else {
        resultOfUnRegisteredCompanies.push(resultOfCompanies[i]);
      }
    }
    this.setState({
      numberOfCompanies: result.count,
      registeredCompanies: resultOfRegisteredCompanies,
      unRegisteredCompanies: resultOfUnRegisteredCompanies,
    });
  };
            ...
            return (
              ...
              >
                <CompanyCell
                  ...
                  viewMode={viewMode}
                  handleClickProfile={() =>
                    this.handleClickProfile(company._id)
                  }
                />
              </div>
            );
    ...
    return (
      <div className="dashboard container" ref={this.fileterPanel}>
        ...
        <div className={`right-panel ${isExpandedSidebar ? "xs" : ""}`}>
          ...
          {filterBarXS}
          {listPanel}
          {/* {unRegisteredCompanies && unRegisteredCompanies.length ? (
            listPanel
          ) : (
            <div className="no-result">{STRINGS.noResults}</div>
          )} */}
