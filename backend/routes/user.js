...
router.post("/forgot-password", async (req, res) => {
  ...
  try {
    let result = await client.query(
      q.Paginate(q.Match(q.Index("findUserByEmailAndActive"), data.email, "1"))
    );
...
router.post("/news/delete", async (req, res) => {
  ...
  try {
    await client.query(
      q.Delete(q.Select("ref", q.Get(q.Match(q.Index("findPostsById"), data.id))))
    );
...
router.post("/delete", async (req, res) => {
  ...
  try {
    ...
    if (isPosts.data.length) {
      await client.query(
        q.Delete(q.Select("ref", q.Get(q.Match(q.Index("findPostsByUserId"), data.id))))
      );
    }
  } catch (error) {
    res.send({ status: 0, message: "Database connection failed!" });
