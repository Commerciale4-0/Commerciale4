...
router.post("/", async (req, res) => {
	if (req.body.id !== "all") {
		try {
			let result = await client.query(
				q.Map(q.Paginate(q.Match(q.Index("findUserById"), parseInt(req.body.id))), q.Lambda("ref", q.Select(["data"], q.Get(q.Var("ref")))))
			);
			let posts = await client.query(
				q.Map(q.Paginate(q.Match(q.Index("findPostsByUserId"), parseInt(req.body.id))), q.Lambda("ref", q.Select(["data"], q.Get(q.Var("ref")))))
			);
			...
			if (result && result.data && result.data.length) {
				...
				res.send({ status: 0, data: "Cannot find the user" });
			}
		} catch (error) {
			console.log(error);
		...
		try {
			let result = await client.query(
				q.Map(q.Paginate(q.Match(q.Index("findUserByActive"), "1")), q.Lambda("ref", q.Select(["data"], q.Get(q.Var("ref")))))
			);
...
router.get("/profile/:id", async (req, res) => {
	...
	try {
		...
		let posts = await client.query(
			q.Map(q.Paginate(q.Match(q.Index("findPostsByUserId"), userId)), q.Lambda("ref", q.Select(["data"], q.Get(q.Var("ref")))))
		);
...
router.post("/delete", async (req, res) => {
	...
	try {
		let isPosts = await client.query(
			q.Map(q.Paginate(q.Match(q.Index("findPostsByUserId"), data.id)), q.Lambda("ref", q.Select(["data"], q.Get(q.Var("ref")))))
		);
