  ...
  try {
    ...
    await Promise.all(
      companies.map(async (company) => {
        let lng = parseFloat(
          company.profile.contact.location.coordinates[0]["$numberDouble"]
        );
        let lat = parseFloat(
          company.profile.contact.location.coordinates[1]["$numberDouble"]
        );
        let data = {
          profile: {
...
router.post("/auth/register", async (req, res) => {
  ...
  try {
    ...
    let result = await Company.find({
      $or: [
        { "profile.account.email": data.account.email },
        { "profile.vat": data.profile.vat },
        { "profile.pec": data.profile.pec },
      ],
    });
...
router.post("/", async (req, res) => {
  ...
  if (filter.tags) {
    ...
    let or1 = filter.tags.en
      ? { "profile.tags.en": { $in: tagsEx } }
      : { "profile.tags.it": { $in: tagsEx } };
    queryOr = [
      ...queryOr,
      { $or: [or1, { "account.permission": 1, $and: tagsExForIntro }] },
    ];
  }
  ...
  if (filter.location) {
    if (filter.location.coordinates && filter.location.radius) {
      ...
      let regionCityFilter = {
        "profile.contact.region": filter.location.region.title,
      };
      if (filter.location.city) {
        addressFilter = [
          new RegExp(`\\(${filter.location.city.short}\\)`, "i"),
        ];
        regionCityFilter = {
          ...regionCityFilter,
          "profile.contact.city": filter.location.city.title,
        };
      } else {
      ...
      queryOr = [
        ...
        {
          $or: [
            { "profile.contact.address": { $in: addressFilter } },
            regionCityFilter,
          ],
        },
      ];
